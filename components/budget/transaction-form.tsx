...
interface Props {
  ...
  onClose: () => void;
  /** Called after a successful add / edit / delete so the page can refresh. */
  onSaved: () => void;
}
...
export function TransactionForm({ open, editing, onClose, onSaved }: Props) {
  const { t } = useLanguage();
  const [pending, startTransition] = useTransition();
  const [deleting, startDelete] = useTransition();

  // Seeded from props via lazy initializers. The parent remounts this
  // component (via `key`) every time the sheet opens, so these run fresh
  // for each new/edit session — no state-syncing effect needed.
  ...
  const [member, setMember] = useState(editing?.member ?? "");
  const [date, setDate] = useState(
    editing ? editing.occurredAt.slice(0, 10) : todayISODate(),
  );
  const [error, setError] = useState<string | null>(null);

  // Lock the body scroll while the sheet is open.
  useEffect(() => {
    if (!open) return;
            ...
            >
              ...
              <label className="block">
                ...
                <input
                  ...
                  placeholder={
                    kind === "income" ? "Dad’s contribution" : "Lunch at Ichiran"
                  }
